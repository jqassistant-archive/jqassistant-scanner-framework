...
public class JavaGenFileScannerPlugin extends AbstractScannerPlugin<FileResource, JavaGen> {
    ...
    @Override
    public JavaGen scan(FileResource item, String path, Scope scope, Scanner scanner) throws IOException {
        LOGGER.info(new Date() + " - Running JavaGenFileScannerPlugin");

        final ScannerContext scannerContext = scanner.getContext();
        Store store = scannerContext.getStore();
        final JavaGenFileDescriptor fileDescriptor = store.addDescriptorType(scannerContext.getCurrentDescriptor(), JavaGenFileDescriptor.class);
        ...
        JavaGen javaGen = compilationUnitMapper.map(scannerContext, compilationUnitContext);
